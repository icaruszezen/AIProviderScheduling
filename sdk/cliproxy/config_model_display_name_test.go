	...
	}{
		{
			...
			got: func() *ModelInfo {
				return buildClaudeConfigModels(&config.ClaudeKey{Models: []config.ClaudeModel{{
					Name: "claude-upstream", Alias: "claude-catalog", DisplayName: "Claude Catalog Name"}}})[0]
			}},
		{
			name: "gemini",
			...
			got: func() *ModelInfo {
				return buildGeminiConfigModels(&config.GeminiKey{Models: []config.GeminiModel{{
					Name: "gemini-upstream", Alias: "gemini-catalog", DisplayName: "Gemini Catalog Name"}}})[0]
			}},
		{
			name: "vertex",
			...
			got: func() *ModelInfo {
				return buildVertexCompatConfigModels(&config.VertexCompatKey{Models: []config.VertexCompatModel{{
					Name: "vertex-upstream", Alias: "vertex-catalog", DisplayName: "Vertex Catalog Name"}}})[0]
			}},
		{
			name: "codex",
			...
			got: func() *ModelInfo {
				return buildCodexConfigModels(&config.CodexKey{Models: []config.CodexModel{{
					Name: "gpt-5.5", Alias: "gpt-5.5", DisplayName: "Codex Catalog Name"}}})[0]
			}},
		{
			name: "xai",
			...
			got: func() *ModelInfo {
				return buildXAIConfigModels(&config.XAIKey{Models: []config.XAIModel{{
					Name: "grok-4.5", Alias: "grok-latest", DisplayName: "xAI Catalog Name"}}})[0]
			}}}
...
func TestBuildConfigModelsDisplayNameFallback(t *testing.T) {
	model := buildClaudeConfigModels(&config.ClaudeKey{Models: []config.ClaudeModel{{
		Name: "claude-upstream", Alias: "claude-catalog"}}})[0]

...
func (p *apiKeyClientProvider) Load(ctx context.Context, cfg *config.Config) (*APIKeyClientResult, error) {
	geminiCount, vertexCompatCount, claudeCount, codexCount, xaiCount, openAICompat, antigravityCount := watcher.BuildAPIKeyClients(cfg)
	if ctx != nil {
		select {
		case <-ctx.Done():
			return nil, ctx.Err()
	...
	return &APIKeyClientResult{
		...
		AntigravityKeyCount:  antigravityCount}, nil
}

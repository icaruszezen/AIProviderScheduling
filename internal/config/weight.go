...
func (cfg *Config) ValidateCredentialWeights() error {
	...
	for index := range cfg.AntigravityKey {
		if errValidate := ValidateCredentialWeight(cfg.AntigravityKey[index].Weight); errValidate != nil {
			return fmt.Errorf("antigravity-api-key[%d].weight: %w", index, errValidate)
		}
	}
